...
<HEAD>
	...
	body {
		<?php
		$fond = DB::queryFirstRow("SELECT * FROM fonds where id=%i", 3 );
		if ($fond["image"] =="") {
			?>

			background: black url(images/bgmax/BckgroundVERT.jpg) repeat-x top center;
	...
	<DIV class="container" style="margin-top: 40px;">
		<DIV class="row">
			<DIV class="col col-md-11  col-xs-12">
				<?php
				$massages = DB::query("SELECT * FROM prestations WHERE Categorie=%i ORDER BY Ordre", 2);
